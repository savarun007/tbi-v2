...
function initializePredictionPage() {
  ...
  function displayResults(results) {
      ...
      const cardHtml = `
        <div class="result-card">
            ...
            <div style="margin-top: 25px; display: flex; flex-direction: column; gap: 10px; text-align: center;">
                ...
                <button id="report-btn" class="summarise-btn" style="background-color: transparent; border: 1px solid var(--primary-color); color: var(--primary-color);" data-diagnosis="${result.prediction}" data-confidence="${result.confidence}">
                    Make a Report
                </button>
            </div>
        </div>
    ...
    // 1. Analyze Button Event Listener
    const analyzeBtn = document.getElementById("analyze-btn");
    if (analyzeBtn) {
      analyzeBtn.addEventListener("click", function () {
        const diagnosis = this.getAttribute("data-diagnosis");
    ...
    // 2. Make a Report Button Event Listener
    const reportBtn = document.getElementById("report-btn");
    const modal = document.getElementById("patient-modal");

    if (reportBtn && modal) {
      reportBtn.addEventListener("click", function () {
        // Save diagnosis and confidence temporarily
        localStorage.setItem(
          "current_diagnosis",
          this.getAttribute("data-diagnosis"),
        );
        localStorage.setItem(
          "current_confidence",
          this.getAttribute("data-confidence"),
        );

        // Save the image source to pass to the report
        const imgSrc = document.getElementById("image-preview").src;
        localStorage.setItem("report_image", imgSrc);
      ...
      document
        ...
        .addEventListener("click", () => {
          ...
          // Save patient data and redirect to report page
